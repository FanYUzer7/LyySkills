...
SuperTrend 是一种基于 ATR（平均真实波幅）的趋势跟踪指标。它通过计算价格通道来确定趋势方向，并在趋势改变时给出明确的买入或卖出信号。
...
其中倍数通常取 2-3，ATR 周期通常取 10。

#### SuperTrend 的基本应用

| 信号 | 含义 |
|------|------|
| 价格上穿 SuperTrend | 买入信号 |
| 价格下穿 SuperTrend | 卖出信号 |
| 趋势持续 | 持有信号 |
...
SAR（Stop and Reverse，抛物线指标）由 J. Welles Wilder Jr. 创建。它的名称来源于"停止并反转"的特性，即当趋势反转时，SAR 会给出平仓并反向开仓的信号。
...
其中：
- EP（极值点）：上涨趋势中的最高价或下跌趋势中的最低价
- AF（加速因子）：初始为 0.02，最大为 0.2
...
Ichimoku Cloud（一目均衡表）是一种复杂但功能强大的技术指标，由日本分析师一目山人在 1930 年代创建。它通过多条线和"云图"来全面分析市场的趋势、支撑阻力位和动量。
...
转换线 = (9日最高价 + 9日最低价) / 2
基准线 = (26日最高价 + 26日最低价) / 2
先驱A = (转换线 + 基准线) / 2 (向后位移26天)
先驱B = (52日最高价 + 52日最低价) / 2 (向后位移26天)
延迟线 = 收盘价 (向后位移26天)
```
...
随机指标（%K 和 %D）是乔治·莱恩（George Lane）创建的，与 KDJ 指标非常相似。它通过比较收盘价与一定时期内的价格区间来判断市场的超买超卖状态。
...
Williams %R（威廉指标）由拉里·威廉姆斯（Larry Williams）创建，与 Stochastic 类似，但计算方式略有不同。它的范围是 -100 到 0。
...
MOM（Momentum，动量指标）是最简单的动量指标之一，它直接计算当前价格与 n 日前价格的差值。
...
成交量分布是一种显示在特定价格水平上成交量的技术工具。它可以帮助识别"高成交量节点"（价格密集成交区）和"低成交量节点"（价格空白区）。
...
A/D（Accumulation/Distribution，累积派发指标）由马克·查金（Marc Chaikin）创建。它通过比较收盘价与当日价格区间的关系来判断资金是流入还是流出。
...
肯特纳通道由查斯特·肯特纳（Chester Keltner）创建，是一种基于 ATR 的价格通道指标。与布林带类似，但它使用 ATR 而不是标准差。
...
通常 EMA 周期取 20，ATR 周期取 10，倍数取 2。

#### Keltner Channel 的基本应用
...
唐奇安通道由理查德·唐奇安（Richard Donchian）创建，是最简单的通道指标之一。它使用过去 n 日的最高价和最低价来构建通道。
...
**趋势确认组合**
- SuperTrend + ADX：双重趋势确认

**择时信号组合**
- Stochastic + RSI：双重超买超卖验证

**突破交易组合**
- Keltner Channel + Volume Profile：突破确认
